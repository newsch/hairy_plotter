...
def calculate_parameters():
    ...
    MAX_WIDTH = SHEET_WIDTH - MARGIN_WIDTH*2  # max width of text in document units
...
def lines_to_gcode(lines: list, align='left'):
    ...
    width = SHEET_WIDTH
    height = (1 + (len(lines) - 1)*(1 + LINE_SPACING))*CHAR_HEIGHT + 2*MARGIN_HEIGHT
    ...
    for i, line in enumerate(lines):
        ...
        if align == 'left':
            ...
            x = SHEET_WIDTH - MARGIN_WIDTH - len(line)*CHAR_WIDTH
        elif align == 'center':
            x = MARGIN_WIDTH + (SHEET_WIDTH - 2*MARGIN_WIDTH - len(line)*CHAR_WIDTH)/2

        y = MARGIN_HEIGHT + (i*LINE_SPACING + i)*CHAR_HEIGHT
